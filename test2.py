...
wrong=np.zeros((300, 500), np.uint8)
imgPil_wrong = Image.fromarray(wrong)   
...
imgPil1 = Image.fromarray(new_img1)   
...
while True:
    if menu==1:
        for k in range (2):
            ...
            control=0
            new_img2=np.zeros((300, 500), np.uint8)
            imgPil2 = Image.fromarray(new_img2)   
            draw2 = ImageDraw.Draw(imgPil2)
            word1=['請輸入參數threshold'+str(recommendation[3*k])+' 門檻值，範圍 0～255','推薦值為'+str(recommendation[3*k+1])+'不推薦超過'+str(recommendation[3*k+2]),'確認後請按下enter','你輸入的值 :'] 
        ...
        new_img3=np.zeros((300, 500), np.uint8)
        imgPil3 = Image.fromarray(new_img3)   
        ...
        new_img4=np.zeros((300, 500), np.uint8)
        imgPil4 = Image.fromarray(new_img4)   
        ...
        start2=np.zeros((300, 500), np.uint8)
        imgPil_start2 = Image.fromarray( start2)   
        ...
        input2=np.zeros((300, 500), np.uint8)
        imgPil_input2 = Image.fromarray(input2)   
        ...
        finish_img=np.zeros((300, 500), np.uint8)
        finish_imgPil = Image.fromarray(finish_img)   
        ...
        finish_img2=np.zeros((300, 500), np.uint8)
        finish_imgPil2 = Image.fromarray(finish_img2)   
        ...
        finish_img3=np.zeros((300, 500), np.uint8)
        finish_imgPil3 = Image.fromarray(finish_img3)   
